...
mod tests {
    ...
    /// SQL, but the fixture still opens a real connection via
    /// `PgPool::connect` against the shared `DATABASE_URL` target —
    /// every test that acquires a real connection carries the crate-wide
    /// serial key, no exceptions, regardless of whether it queries.
    #[tokio::test]
    #[serial(hort_pg_db)]
