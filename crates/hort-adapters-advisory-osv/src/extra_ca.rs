//! Dialect-specific extra-CA helper for `hort-adapters-advisory-osv`.
//!
...
//! `hort-adapters-upstream-http/src/extra_ca.rs`. The function is per-
//! adapter (not a shared helper) on purpose: every adapter that opens
//! TLS must build via `reqwest::Client::builder()` (ADR 0010), and
//! keeping the helper local keeps the dep graph local —
//! `hort-adapters-advisory-osv` does NOT depend on
//! `hort-adapters-upstream-http`.
...
mod tests {
    ...
    /// used by `hort-adapters-upstream-http`. Using rcgen ensures the cert
    /// is a fully-valid X.509 CA that rustls / reqwest can load as a
    /// trust anchor.
    fn make_ca_pem() -> String {
        let mut params = rcgen::CertificateParams::default();
        params.distinguished_name.push(
            rcgen::DnType::CommonName,
            "advisory-osv extra-ca-test root CA".to_string(),
        );
