//! Dialect-specific extra-CA helper for `hort-adapters-provenance-sigstore`.
//!
...
//! adapter (not a shared helper) on purpose: ADR 0010 requires every
//! adapter that opens TLS to build via `reqwest::Client::builder()`,
//! and keeping the helper local keeps the dep graph local — this
//! adapter does NOT depend on any other adapter crate.
//!
//! The ONLY live HTTP client this adapter builds is the periodic TUF
//! trust-root refresh (ADR 0027). The verify path is fully offline (the
//! bundle carries the Fulcio chain + Rekor SET; verification runs against
//! a cached, injectable trust root). So this helper is wired only into
//! the trust-root *refresh* client, never the verify path. The
//! `sigstore` crate's own built-in TUF (`SigstoreTrustRoot::new`) uses
//! `reqwest::Client::new()` internally (the exact ADR 0010 anti-pattern)
//! and is deliberately NOT used here; instead the refresh fetches the
//! trusted-root JSON with this builder and hands the bytes to
//! `SigstoreTrustRoot::from_trusted_root_json_unchecked`.
...
/// sentinel format. Same shape as the upstream-http helper; duplicated
...
mod tests {
    ...
    /// the OSV adapter uses. Guarantees a fully-valid X.509 CA that rustls
    /// / reqwest accept as a trust anchor.
    fn make_ca_pem() -> String {
        let mut params = rcgen::CertificateParams::default();
        params.distinguished_name.push(
            rcgen::DnType::CommonName,
            "provenance-sigstore extra-ca-test root CA".to_string(),
        );
    ...
    /// that prefix again. Mirrors the upstream-http test of the same name.
    #[test]
