...
//!
//! The comment/string stripper, the tokenizer and the table-name parser are
//! shared with the sibling `expand_contract_guard` target via
//! `tests/sql_scan/mod.rs` — one implementation of the matcher discipline,
//! so the two guards cannot drift into differing notions of what a SQL
//! identifier is. What stays HERE is what encodes this guard's policy: the
//! sensitive-table list, the destructive-shape matcher, the `_check`-only
//! redefinition allow-list, and the self-checks that pin them.
...
use sql_scan::{
    line_of, migrations_dir, parse_table_name, strip_comments_and_strings, tokenize, Token,
};
...
// ---------------------------------------------------------------------------
// Statement matcher. The lexer it walks (comment/string stripping,
// tokenizer, table-name parser) lives in `tests/sql_scan/mod.rs`.
// ---------------------------------------------------------------------------
