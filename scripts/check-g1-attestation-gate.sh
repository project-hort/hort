...
# logging / GDPR Art. 17(3)(b) attestation may publish the F-2 audit-
# log-tamper-evidence claim. The claim is honest ONLY when the F-2 (and
# F-5) deliverables are demonstrably `Active` — i.e. their telemetry
# rows are catalogued and their named regression tests exist — AND the
# wording stays at the spec-§5 "tamper-EVIDENT" bar (never the
# unqualified "tamper-proof"/"tamper-resistant" self-claim).
#
# The predicate (all must hold; any failure → non-zero exit):
#
#   (a) docs/metrics-catalog.md catalogues BOTH the F-2 telemetry row
#       `hort_event_chain_verify_total` AND the F-5 telemetry row
#       `hort_advisory_ingest_count` (the §2.4 G1 metric pair).
#
#   (b) The named F-2 + F-5 regression tests are present BY NAME in
#       their source files (the test-inventory half of §2.4):
#         F-2: tampered_row_fails_verification
...
#         F-5: iter_zip_entries_trusted_bulk_config_completes_all_for_large_archive
...
#
#   (c) IF any file under docs/compliance/ actually PUBLISHES the F-2
#       audit-log tamper-evidence self-attestation (recognised by an
#       F-2-chain self-claim marker: a "tamper-{proof,resistant,
...
#           "tamper-proof" or "tamper-resistant" (spec §5 honesty
#           caveat — the gate's whole reason to exist: don't publish a
#           claim stronger than the delivered property).
#       A compliance file that only cites the NIS2 *legal mandate*
#       wording ("…interpret this as a tamper-resistant logging
#       mandate") with NO F-2-chain self-claim marker is the
...
#   - locally before pushing compliance/catalog/F-2/F-5 changes
#
...
if ! catalog_has_row "hort_event_chain_verify_total"; then
    fail "F-2 telemetry row \`hort_event_chain_verify_total\` missing from ${catalog}."
fi
if ! catalog_has_row "hort_advisory_ingest_count"; then
    fail "F-5 telemetry row \`hort_advisory_ingest_count\` missing from ${catalog}."
fi

# ---------------------------------------------------------------------
# (b) Named F-2 / F-5 regression tests present by name in their source.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# (c) Conditional wording guard on a published F-2 self-attestation.
# ---------------------------------------------------------------------
...
# The unqualified self-claim spec §5 forbids in a published attestation.
# Only an *affirmative* self-claim is forbidden; a *negated/disclaimer*
# use is the honest wording (spec §5's own verbatim text is
...
#   - the spec §5 verbatim "tamper-evident, not tamper-{proof,resistant}";
...
# (the operator rephrases to the spec-§5 verbatim form, which is the
# wording they should publish anyway).
...
for f in "${attestation_files[@]}"; do
    ...
    if [[ "${has_chain_marker}" -eq 0 ]]; then
        # tamper word(s) but no F-2-chain self-claim marker → this file
        # only cites the NIS2 legal mandate, not hort's own
        # delivered property. Pre-attestation state: (c) vacuous.
        continue
    fi

    # This file publishes an F-2 self-attestation. Enforce the spec-§5
    ...
    if ! grep -q 'tamper-evident' "${f}"; then
        fail "${f} publishes an F-2 audit-chain attestation but does not use \
the qualified \"tamper-evident\" wording (spec §5 honesty caveat)."
    fi
    ...
    if [[ -n "${bad_lines}" ]]; then
        fail "$(printf '%s publishes an F-2 audit-chain attestation with an affirmative unqualified self-claim "tamper-proof"/"tamper-resistant" (spec §5 forbids it — the delivered property is tamper-EVIDENT; only a negated/disclaimer or NIS2-mandate-citation use is honest):\n%s' "${f}" "$(printf '%s' "${bad_lines}" | head -5)")"
    fi
done

echo "G1-attestation-gate: OK (catalog F-2/F-5 rows present; named F-2/F-5 regression tests present; docs/compliance attestation wording within the spec-§5 tamper-evident bar)."
exit 0
