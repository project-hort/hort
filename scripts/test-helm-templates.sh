...
test-values-extra-ca-with-cm.yaml|extra-ca-bundle|13|Recipe A: server mount+vol (2) + worker mount+vol (2) + scrub mount+vol (2) + quarantineReleaseSweep mount+vol (2) + scanRowRetentionSweep mount+vol (2) + prefetchRowRetentionSweep mount+vol (2) + checksum annotation (1)
test-values-extra-ca-with-cm.yaml|defaultMode: 0444|6|Recipe A: read-only 0444 ConfigMap volume on server + worker + scrub + quarantineReleaseSweep + scanRowRetentionSweep + prefetchRowRetentionSweep
...
test-values-extra-ca-with-secret.yaml|extra-ca-bundle|12|Recipe A-Secret: server mount+vol (2) + worker mount+vol (2) + scrub mount+vol (2) + quarantineReleaseSweep mount+vol (2) + scanRowRetentionSweep mount+vol (2) + prefetchRowRetentionSweep mount+vol (2); NO checksum annotation (Secret path)
test-values-extra-ca-with-secret.yaml|defaultMode: 0444|6|Recipe A-Secret: read-only 0444 Secret volume on server + worker + scrub + quarantineReleaseSweep + scanRowRetentionSweep + prefetchRowRetentionSweep
...
test-values-extra-ca-with-secret.yaml|corporate-ca-bundle-secret|6|Recipe A-Secret: the Secret name renders in the volume secretName on server + worker + scrub + quarantineReleaseSweep + scanRowRetentionSweep + prefetchRowRetentionSweep
...
test-values-cronjobs.yaml|^kind: CronJob$|15|fifteen CronJobs render — the eleven scheduledTasks.adminTasksEnabled-gated admin-task entries (noop, staging-sweep, cron-rescan-tick, advisory-watch-tick, eventstore-checkpoint, replay-seen-prune, scanner-registry-prune, retention-evaluate, retention-purge, eventstore-archive, verify-event-chain — the last now default-on, ADR 0057) PLUS the always-on dsn-direct quarantineReleaseSweep (gated on scheduledTasks.quarantineReleaseSweep.enabled, default true) PLUS the always-on dsn-direct CAS scrub (gated on scheduledTasks.scrub.enabled, default true) PLUS the always-on dsn-direct scanRowRetentionSweep (gated on scheduledTasks.scanRowRetentionSweep.enabled, default true) PLUS the always-on dsn-direct prefetchRowRetentionSweep (gated on scheduledTasks.prefetchRowRetentionSweep.enabled, now default true: its terminal oci-index-child-ingest rows accrue on any OCI proxy repository with no per-repo opt-in, so a default-off sweep left them retained permanently — the same day-one-rows argument that has scanRowRetentionSweep shipping enabled). wheelMetadataBackfill stays default-disabled (a one-shot retrofit, not an accruing-row sweep) and does NOT count.
...
test-values-cronjobs.yaml|^  concurrencyPolicy: Forbid$|15|every CronJob (incl. the 3 retention + the always-on quarantineReleaseSweep + the always-on scrub + the always-on scanRowRetentionSweep + the now-default-on prefetchRowRetentionSweep + scanner-registry-prune + the now-default-on verify-event-chain, ADR 0057) sets concurrencyPolicy: Forbid (single-active layer 1; worker semaphore is layer 2)
...
test-values-worker-metrics-worker-disabled.yaml|HORT_WORKER_METRICS_BIND|0|worker.enabled=false renders NO worker Deployment at all, so the /metrics bind env never renders despite worker.metrics.enabled=true left set — directive 118 rev2's fixed gap
test-values-worker-metrics-worker-disabled.yaml|^kind: NetworkPolicy$|0|worker.enabled=false with networkPolicy.enabled=false renders NO NetworkPolicy at all — the worker-scoped scrape policy stays skipped (no worker pod to protect) and the app-pod/Job policies stay off via the explicit networkPolicy.enabled=false, so this combination must NOT trip the worker-metrics fail guard
test-values-worker-metrics-worker-disabled.yaml|app.kubernetes.io/component: worker|0|worker.enabled=false renders NO worker-labelled resource at all (Deployment, ConfigMap, ServiceAccount, NetworkPolicy) — worker.metrics.enabled=true is inert with no worker pod to attach a listener to
...
test-values-dex.yaml|value: "https://registry.example.com/dex"|5|auth.dex.enabled overrides HORT_OIDC_ISSUER_URL to auth.dex.issuerUrl on the server Deployment AND the four DSN-direct runtimeEnv CronJobs (scrub + quarantineReleaseSweep + scanRowRetentionSweep + prefetchRowRetentionSweep)
test-values-dex.yaml|^ +value: "hort-cli"$|5|auth.dex.enabled overrides HORT_OIDC_AUDIENCE to auth.dex.cliClientId (Dex mints the cli-flow token; its aud is the Dex client id, NOT auth.oidc.audience) — on the server Deployment AND the four DSN-direct runtimeEnv CronJobs; mirrors the issuer override and the Ansible flavor
...
test-values-worker-metrics-no-networkpolicy.yaml|worker.metrics.enabled=true with networkPolicy.enabled=false|worker.metrics.enabled=true with networkPolicy.enabled=false is the one combination nothing else rejects — the listener turns on with no NetworkPolicy protecting it at all — so worker-networkpolicy.yaml's unconditional `fail` guard must reject it at helm template, naming both values
