...
- [ ] **Destructive DDL in the same release that removed the last code reference** — a migration that drops/renames/narrows a schema identifier may ship only in a release strictly *after* the last release whose code referenced it (expand/contract, ADR 0030 amendment (c)). `expand_contract_guard` rejects it via `migrations/CONTRACTIONS.toml` version arithmetic; the fix is to schedule the contraction one release later, never to edit the manifest into passing. Every new manifest entry carries a mandatory reviewer step: `git grep <identifier> v<reference_removed_in>` confirms no SQL at that tag names it (the hermetic guard cannot see past releases).
...
- [ ] **Single-ended cross-artifact lifecycle trigger** — a standing cross-artifact lifecycle dependency ("artifact A's state change decides artifact B's state") must name a trigger at **both** ends: one fired by A's change, one fired by B's arrival/change. A single-ended trigger is correct only for the arrival order its author had in mind, and silently strands every other order. The canonical exemplar: the provenance-clearance cascade shipped with only the subject-end trigger (verify-time cascade to already-ingested constituents); every constituent that happened to arrive *after* its subject verified had no clearance path and stranded under `Required` until the late-joiner (constituent-end) trigger was added (#135). Register every such dependency in `hort_app::lifecycle_dependency_registry::DEPENDENCY_EDGES`, both trigger ends named as real code sites — the `lifecycle_dependency_registry_guard` structural test (`crates/hort-app/tests/`) rejects an entry with an empty trigger-site field. Applies beyond provenance: any future subject⇄constituent, parent⇄child, or policy⇄artifact dependency. Both ends should resolve the decision from the SAME authority where possible (sharing one implementation, as `ProvenanceCascade` does for both cascade directions) so the two ends structurally cannot disagree about what is covered. (see ADR 0039 §12)
...
| `ScanPolicy.enforcement: record` | Makes a blocking scan verdict recorded-but-not-enforced: findings + `PolicyEvaluated(Fail)` persist, no `ArtifactRejected`, and release runs on the distinct `ScanRecorded` authority | `scan_backends: []` — **vacuous, not collapsing**: with no scanner there is no verdict to record or enforce, exactly as `severityThreshold` is already vacuous under a waiver. `trust_upstream_publish_time = true` — shortens latency-to-release but collapses no invariant: `record` alone has already declared the scan verdict non-gating, so the combination adds no relaxation the operator did not make explicitly, and the scan still runs and records evidence the tighten direction can act on. `provenance_mode: required` / active curation rules — **no interaction**: `ScanRecorded` carries the same provenance AND-precondition as the other timer authorities, and `record` un-gates the scan axis only. Neither combination is apply-time rejected. |
