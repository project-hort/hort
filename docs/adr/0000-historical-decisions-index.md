...
| Combined real-verifier provenance E2E | Provenance verification is composition-proven (in-crate fixture tests + offline cosign smoke). The first real-verifier worker→release-gate E2E now exists: `scripts/native-tests/scenarios/quarantine/provenance-push-then-sign.sh` drives the keyed push-then-sign round-trip (hosted OCI + `required` + `cosign-key`) — push→held (anonymous manifest GET 503, write-authorized HEAD exists)→`cosign sign --registry-referrers-mode=oci-1-1`→`ProvenanceVerified`→release+pull after the window, plus the negative never-signed→terminal `Rejected{Unsigned}` at expiry (issue #13, hold-until-signed amendment on [0027](0027-artifact-provenance-verification.md)/[0039](0039-keyed-provenance-verification.md)). **Still OPEN** — the scenario runs only when a CI job provisions `cosign`+`buildah` in the client image, a keyed key whose public half is the worker's registered `cosign-key`, and a short-`quarantineDuration` `required`/`cosign-key` gitops repo (else it self-skips, never a false pass); and it covers only the **keyed** backend. The **keyless (Fulcio/Rekor) bundle** push-then-sign path — which shares the same S1/S3/S4 lifecycle and release gate — is not yet exercised end to end. The scenario also empirically settles the design's cosign HEAD-vs-GET assumption (§2 S2): its step [3] runs with only the write-authorized manifest-**HEAD** exemption in place, so a pass confirms keyed `cosign sign` resolves via HEAD (Item 3's exemption sufficient); a step-[3] failure on a manifest-GET 503 is the signal to extend Item 3's exemption to a write-authorized manifest GET (fast-follow). Close this row when the keyless path is covered too (relates [0027](0027-artifact-provenance-verification.md)). |
