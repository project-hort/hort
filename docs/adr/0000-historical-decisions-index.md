...
| Combined real-verifier provenance E2E | The real-verifier worker→release-gate E2E runs and passes: `scripts/native-tests/scenarios/quarantine/provenance-push-then-sign.sh` drives the keyed push-then-sign round-trip against a provisioned compose stack (cosign in the client image, committed test keypair, worker `cosign-key` backend, `required`/`cosign-key` gitops repo, sweep-ticker) — index push→held (anonymous GET 503; write-authorized HEAD **and GET** serve — the settled HEAD-vs-GET answer: keyed cosign resolves the subject by GET)→`cosign sign --registry-referrers-mode=oci-1-1` (Sigstore v0.3 DSSE bundle)→`ProvenanceVerified`→clearance cascade→release+pull, plus never-signed→terminal `Rejected{Unsigned}` at expiry ([0027](0027-artifact-provenance-verification.md)/[0039](0039-keyed-provenance-verification.md)/[0043](0043-oci-image-index-support.md)). The scenario also runs in **native-token mode** (`run.sh --compose-overlay=native-tokens`): a gitops PAT-only ServiceAccount (`provenance-ci`, read+write serviceAccount-subject grants) drives the real `/v2/auth` token dance, so cosign's subject read rides a **pull-scoped capability JWT** and the granted-write hold-read exemption ([0039](0039-keyed-provenance-verification.md) §10, issue #13) is exercised end to end — the pull-scoped-cap × write-grant gap is CLOSED. **Still OPEN — two coverage gaps.** (1) The **keyless (Fulcio/Rekor) bundle** push-then-sign path — same S1/S3/S4 lifecycle and release gate — is not exercised end to end. (2) A **private-repo E2E variant**: the fixture repo is `isPublic: true`, so the scenario's anonymous read legs ride public access and the pull-scoped cap never has to *grant* the read. **The defect this gap named is CLOSED** by the OCI read-path challenge initiative ([0045](0045-oci-read-challenge-before-anti-enum.md)): previously, on a private repo, a write-only principal pushed fine while every anonymous read — including cosign's subject preflight — anti-enumerated straight to a bare 404 with no challenge, so a challenge-based retry never had a signal to act on; anonymous denied reads on `/v2/*` now get 401 + `WWW-Authenticate` in production auth modes, so a credentialed cosign retry can complete instead of silently failing. **Still OPEN**: no E2E scenario yet drives cosign push-then-sign against a *private* repo end to end — the `/v2/auth` mint narrows `pull,push` to `push` (permissions are flat; `write` does not imply `read`), so the signer's subject-read leg still needs its own explicit `read` grant, and that combination is untested. Close when both this variant and the keyless (1) variant run. |
