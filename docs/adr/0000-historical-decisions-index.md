...
| ADR | Decision |
|---|---|
| [0005](0005-wasm-format-modules-capability-taxonomy.md) | WASM format modules with a capability-group taxonomy; **VersionDiscovery** (issue #58) realised as an accessor (`FormatHandler::version_discovery() -> Option<&dyn VersionDiscovery>`, default `None`) over eight upstream-version-discovery/download-URL-resolution members — npm/Cargo/PyPI participate, OCI/Maven/Helm inherit `None` |
...
| [0046](0046-admintasks-operator-provisioned-sa-loud-preflight.md) | `adminTasksEnabled` installs are operator-provisioned + loud-preflight, not chart-provisioned: the chart never writes the `cronjob-tasks` SA/grant gitops (operator owns it via the audited apply path — ADR 0012/0037/0044); `issue-svc-token --require-authority` verifies the backing global `admin_task_invoke` grant via the runtime `RbacEvaluator` grants-leg and fails the install with copy-paste YAML when the SA **or** grant is absent (was a silent CronJob 403); destructive kinds stay disabled-by-default + docs-only (admin-session-only, ADR 0038 §4; token-seam + approval-workflow deferred to the 0038 follow-on) |
...
| [0048](0048-release-branch-staging-strategy.md) | Release/branch/staging model: `develop` → `test/vX.Y.Z-alpha.N` (throwaway, pushed for staging, **never merged back** — no version-bump commit on `develop`/`main`) → `main` (public, version-fixed promotion MR). Existing throwaway-tag mechanism kept; internal pre-releases renamed `beta`/`rc` → **`alpha`** (semver `alpha<beta<rc` reserves a future public track). Staging is continuous/multi-source (develop + test/* + main); alpha **builds** images+chart but to the **internal** registry only (GitLab `build-images:*`/`helm` regex widened to `alpha`), **excluded** from public ghcr (GitHub `docker-publish.yml`). `ready-for-staging`/`in-uat`/`closed` are resting states decoupled from `main` (one release batch-closes many). Source: issue #25 |
| [0049](0049-regenerate-attribution-on-any-dependency-change.md) | **ANY** dependency-graph change (bump, `cargo update`, add/remove, any non-workspace `Cargo.lock` delta) MUST regenerate `THIRD-PARTY-LICENSES.{md,json}` (`scripts/regenerate-attribution.sh`, in the sandbox) and commit them in the **same** change/MR; release-workspace-version-only bumps are exempt (no third-party change). Authoring-time discipline is now the first line of defence because #30 rescoped `security:attribution-sync` to release-relevant pipelines only (it no longer runs per-MR) — the gate stays the fail-closed release backstop, not re-widened. A dep-changing MR without the regen is a review hard block. Root cause: #28 (spin bump) relied on the per-MR check #30 had already removed; surfaced by #31 / #33. Source: issue #33 |
| [0050](0050-pull-dedup-leader-bound-exceeds-storage-put-timeout.md) | `HORT_PULL_DEDUP_LEADER_TIMEOUT_SECS` (600s) must stay comfortably **above** `HORT_STORAGE_PUT_TIMEOUT_SECS` (300s): the coalesce leader's closure is `fetch + ingest_verified`, whose slowest leg is the storage `put`, so a leader bound at or below the storage bound abandons leaders that are still working — most often on exactly the large multi-arch pulls coalescing protects. Corollaries: `follower_wait` (300s) stays **below** the leader deadline; and coalesce bounds **fail fast and retry, never fail open** (evict + `Failed{Timeout}` terminal → fresh election), because the path sits in front of ingest where [0007](0007-quarantine-release-authority.md)'s fail-closed release authority is established. Tuning either knob is a paired decision — the relationship, not either value, is what matters. Observable via `leader_timeout`; note `hort_pull_dedup_wait_seconds` does **not** cover a wedge (it records only after the closure returns). Root cause: #53 (openbao layer unpullable for days) → #55 (wedged leader, fixed in !168). Source: issue #59 |
...
| Maven Phase-2 prefetch | Scheduled/transitive prefetch for Maven is **deferred** ([0032](0032-maven-gradle-multi-file-handler.md)). `MavenVersionOrdering` exists but is consumed only by the Maven serve/builder path; Maven is unreachable in `self_service_prefetch_use_case` today (the `hort-formats-upstream` `UnsupportedFormat` guard), and `prefetch_tick::ordering_for_format` returns `None` for Maven. **The wildcard-footgun half is now CLOSED (issue #58, [0005](0005-wasm-format-modules-capability-taxonomy.md)'s VersionDiscovery realisation note).** `self_service_prefetch_use_case::ordering_for_format` no longer has a `_ => &NpmSemverOrdering` default — it returns `Option<&dyn VersionOrdering>`, `None` for every non-participant, and `process_item` fails closed (`PrefetchItemError::Internal`) rather than silently mis-ordering if that `None` is ever reached; participation is now structural (`FormatHandler::version_discovery()`), so a future format cannot reach an ordering lookup at all without first, consciously, implementing `VersionDiscovery`. **Still deferred — the feature itself.** Enabling real Maven prefetch still requires, in one coupled change: the Maven arm on **both** `ordering_for_format` sites (`prefetch_tick.rs` *and* `self_service_prefetch_use_case.rs`), **plus** the `hort-formats-upstream` dispatch, **plus** `MavenFormatHandler::version_discovery()` returning `Some` with a real `VersionDiscovery` impl (Maven has neither cargo's `config.json`/`dl` nor npm's `dist.tarball` shape, so it needs its own download-URL resolution member and a Maven arm in `prefetch_ingest.rs`'s dispatch). |
...
| Destructive-op approval workflow | **DEFERRED INDEFINITELY (2026-07-20, maintainer decision on issue #2) — the security property it was framed around is already met, so this buys convenience, not safety.** The design: cron *proposes* a destructive op non-destructively (retention-purge / eventstore-archive / retention-evaluate); a fresh admin CliSession *confirms*; the worker *executes* attributed. **Why deferred:** the interim posture — destructive CronJobs disabled-by-default, run by hand with a Dex CliSession — already achieves zero standing destructive privilege, because no machine runs them at all. What the workflow adds is running them **unattended**. That costs domain events, an app-layer proposal use case, an adapter proposal store, an HTTP list/approve/reject surface, worker approval-triggered execution, a Helm cron change, a new ADR and an [0020](0020-single-flight-seal-pool-backstop.md) amendment — plus a mandatory **security co-review**, since it changes the destructive-execution trigger and touches the seal-pool single-flight invariant. **Revisit trigger:** an operator genuinely needs a destructive op to run unattended on a schedule — i.e. the manual cadence becomes a real burden rather than a hypothetical one. Not closed: the design thinking is sound and worth keeping. (ADR [0038](0038-admin-identity-model.md) follow-on 1) |
