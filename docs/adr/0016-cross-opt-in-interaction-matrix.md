...
- **`ScanPolicy.provenance_mode: Required`** × **`ScanPolicy.quarantine_duration_secs` shorter than a client push** — **dissolved for constituents; unchanged (and as-asked) for subjects.** A short window was an *availability* hazard for subjects and a *correctness* hazard for constituents, and only the second was a defect:
  - For a **subject** (an OCI manifest or index — the digest cosign signs), a window shorter than the signer's round-trip shortens the observation window exactly as the operator asked. An expiry with no signature is a real, terminal `Rejected{Unsigned}` verdict about a row that *could* have been signed. Unchanged.
  - For a **constituent** (an OCI config/layer blob — a row that can never carry an attestation of its own), the same expiry used to produce the same terminal verdict, even though no signature for that row will ever exist. Because clients push blobs *before* the manifest that references them, the blob also had no inbound `content_references` edge yet, so the zero-window descendant hold did not catch it either — and `Rejected` is terminal for `reevaluate`, `waive`, the admin override and the cascade alike. Servability of a correctly signed image therefore depended on client push order. The hold predicate now includes `FormatHandler::is_provenance_constituent`, so an unsigned constituent is **held** (`held_pending_subject`) rather than self-rejected, and is cleared only by its subject — the existing verify-time cascade or the ingest-time late-joiner self-clear.

  No apply-time rejection rule: the combination grants no release authority in either direction (a held constituent serves nothing, and clearance authority is unchanged), so there is nothing to fail-closed-reject. An apply-time floor on `quarantine_duration_secs` was considered and rejected — it moves the race rather than removing it, since no floor can be guaranteed longer than an arbitrary client's push.
