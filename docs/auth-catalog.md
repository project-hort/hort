...
- **Purpose / allowed use:** short-lived interactive CLI session, optionally admin-capable. **The only native-token kind that carries non-admin claims** (ADR 0012's claimless invariant scopes to long-lived static tokens; CliSession is ≤15 min and IdP-backed — ADR 0013); PAT/SA/Refresh stay claimless. This is what lets a `GrantSubject::Claims` grant authorize the CliSession-gated discovery endpoint below.
- **Restrictions & caps:** default **15 min (900 s)**; admin-capable bounded by the SAME **≤15 min (900 s)** hard cap (`crates/hort-app/src/use_cases/api_token_use_case.rs` clamp + `cli_session_signing.rs`) — a signed JWT is non-revocable-until-`exp` by construction, so the TTL is the revocation floor. Minted via PKCE-S256 mediated login (RFC 8252 loopback redirect + RFC 7636 S256 — `crates/hort-cli/src/auth/loopback.rs`). Endpoint `GET /api/v1/repositories/{repo_key}/discovery/versions/{package}` requires `TokenKind::CliSession` alone (token-kind gate in `crates/hort-app/src/use_cases/discovery_use_case.rs` — cost-shape rationale: an amplification surface requires a time-capped credential). `POST /api/v1/repositories/{repo_key}/prefetch` accepts CliSession too, but is **not** CliSession-exclusive — see Entry 4's prefetch note (issue #80; `docs/ci/hort-quarantine-integration.md` designs a non-admin `ServiceAccount` as the CI prefetch caller).
...
- **Restrictions & caps:** the `kind: ServiceAccount` envelope is **identity + federation binding only** (`metadata.name`, `federatedIdentities`, `fallbackRotation` — no `role`, no `repositories`; an envelope still carrying those fields fails apply at parse via `deny_unknown_fields`, `crates/hort-config/src/service_account.rs`); authority exclusively via explicit `GrantSubject::User(backing_user_id)` grants (ADR 0012) — scoped `Read`/`Write`/`Curate`/global `AdminTaskInvoke` expressed as gitops `serviceAccount`-subject `PermissionGrant`s that resolve to `GrantSubject::User(backing_user_id)` at apply (ADR 0037); admin forbidden at three independent points: `validate_permission_grant` hard-rejects a `serviceAccount` subject with `permission: admin` at apply-parse (`crates/hort-config/src/permission_grant.rs`), the apply linter denies the SA-provenance exemption for `Admin` (`direct_user_sa_owned_admin_is_rejected` pins it), and `issue-svc-token` rejects `--permission=admin` (ADR 0038); self-mint forbidden. **Issuance caps:** the fallback-rotation reconciler (`crates/hort-app/src/task_handlers/service_account_rotation.rs`) mints with `declared_permissions` = a snapshot of the SA's effective grants at issuance (`RbacEvaluator::service_account_cap_permissions`; empty for a zero-grant SA — the token holds no authority; staleness bounded by `rotationInterval`, the next tick re-snapshots) and `repository_ids` unset (repo scoping is enforced by the live grants leg); `issue-svc-token` mints with the operator-declared `--permission` cap (admin rejected) — a deliberately different trust posture: an interactive admin declaring intent vs. the unattended snapshot; the federation exchange's cap is Entry 6's.
- **Protection:** at rest — Argon2id; in transit — TLS; blast-radius — User-subject grants only, never claim-mapped.
- **Allowed call paths / surfaces:** all API surfaces; issuance via the admin `issue-svc-token` mint, the fallback-rotation reconciler's system mint, and the federation exchange (Entry 6).
- **Enforcement owner:** `Hort-enforced`.
- **Status:** `Active`.
...
- **Self-service prefetch note (issue #80):** `POST /api/v1/repositories/{repo_key}/prefetch` (Entry 3's amplification-surface endpoint) admits a ServiceAccount bearer — not previously true — **iff** its resolved grants include `Permission::Read ∧ Permission::Prefetch` on the target repository (`crates/hort-app/src/use_cases/self_service_prefetch_use_case.rs`'s Gate 1 + Gate 2). This corrects an implementation-drift bug: `docs/ci/hort-quarantine-integration.md` designs the CI prefetch caller as exactly this — a read+prefetch-only, non-admin ServiceAccount bearer, its cap ADR-0044-snapshotted at federation exchange (Entry 6) — but the endpoint's token-kind gate rejected every non-CliSession caller regardless of grants, a spec/implementation mismatch (prod evidence: issue #80, `self-service prefetch denied: token kind is not CliSession` on every same-repo CI run). The discovery-versions endpoint's CliSession-only gate is unaffected — the design doc speaks only to prefetch, and no other endpoint's token-kind gate changed.
...
- **Restrictions & caps:** short-lived minted bearer; SA selected by `issuer_name` + non-empty `FederatedIdentity.claims` subset match. **Cap:** the minted bearer's `declared_permissions` is a snapshot of the SA's effective grants at exchange time — the distinct permission set of the backing user's `PermissionGrant`s from the live evaluator (`RbacEvaluator::service_account_cap_permissions`, the same enumeration the request-time gate walks) — intersected with the RFC 8693 `scope` parameter when present (a requested scope of which the SA holds nothing is denied `403 access_denied` / `cap_exceeds_authority`, mirroring the CliSession branch; an absent scope applies no narrowing). `repository_ids` stays unset — repo scoping is enforced by the live grants leg at request time. A zero-grant SA mints a bearer with an empty-permissions cap: the token authorizes nothing (fail-closed by intersection, not an issuance error). Freshness: grants added after mint enter the next exchange's cap (CI exchanges per job); revocations bite outstanding tokens immediately through the live grants leg. **Anti-replay:** every accepted JWT is atomically claimed in the durable `jwt_replay_seen` seen-set **before** any token is minted (`ReplayGuardPort` — `crates/hort-domain/src/ports/replay_guard.rs`, Postgres impl `crates/hort-adapters-postgres/src/replay_guard_repo.rs`); a second presentation of the same `jti` (or `(iss,sub,iat,exp)` composite) within its TTL window is denied — no bearer is minted on a replay. Per-`OidcIssuer` `require_jti` trust knob (**default `true`** — secure-by-default; a field-less issuer config means `require_jti=true`, so jti-less JWTs from it are rejected). Setting `require_jti=false` opts an issuer into the weaker `(iss,sub,iat,exp)` composite fallback, which mis-classifies as a replay any two genuinely-distinct JWTs the IdP mints for the same `sub` within the same `iat` second *and* `exp` (documented false-positive cost of the opt-down).
...
- **Mandatory guardrails (ship gates — all met):** [OWASP A07/A01 · NIS2 21(2)(i),(j),(d) · CRA I(1),(2)(a) · BSI ORP.4] **JWT replay seen-set — MET** (`ReplayGuardPort` + durable `jwt_replay_seen` + per-issuer `require_jti` on `crates/hort-domain/src/entities/oidc_issuer.rs`, checked before mint, fail-closed); **`aud`→SA binding — MET** (per-`FederatedIdentity` explicit `audience` bound to `claims.audience`/required `aud`, consulted in SA selection — `collect_sa_matches` in `crates/hort-http-core/src/handlers/exchange.rs` — and `hort_fed_sa_match_total{result="denied_audience"}`; a repo/project-scoped FI that pins no further discriminator additionally triggers a non-blocking apply-time under-constrained-issuer WARNING, a separate advisory control, not this ship gate); **claims fail-closed on cardinality AND content — MET** (apply-time hard reject in `validate_service_account`, `crates/hort-config/src/service_account.rs`, for BOTH an empty `claims` map — "any JWT from this issuer can assume me" — and a non-empty map with no subject-identifying claim (`sub`/`repository`/`project_path`; `aud`/`ref`/`environment`/`workflow` refine a subject but never identify one, so a map built only from those is equally a privilege-escalation footgun on a shared issuer); layered with a runtime defense-in-depth check in `evaluate_fi`, `crates/hort-http-core/src/handlers/exchange.rs`, which returns `NoMatch` for either shape on a stored row that slipped past apply; the cardinality half is additionally enforced at the storage boundary — DB `CHECK (jsonb_typeof(claims)='object' AND claims <> '{}')` in `migrations/011_gitops_machine_identity.sql`, `TryFrom<FederatedIdentityRow>` reject in `crates/hort-adapters-postgres/src/mappers.rs`; `hort_fed_sa_match_total{result="denied_empty_claims"}`). No ship gate remains open on this path.
...
- **Challenge posture on `/v2/*` (ADR 0045).** A repo-level read denial now depends on whether a credential was ever presented, not only on the caller's authority: **anonymous** (no credential presented — an *invalid* one already 401s at the middleware) gets a **uniform 401 + `WWW-Authenticate`** challenge (Bearer + this entry's `/v2/auth` realm when the OCI signing key is wired, legacy `Basic realm="hort"` otherwise) over both {private-existing, nonexistent} — byte-identical modulo the `scope=` echo of the caller's own path, preserving ADR 0021's anti-enumeration guarantee under the new representation; **authenticated** callers keep the unchanged `NAME_UNKNOWN` 404 anti-enum envelope over the same set. Anonymous non-safe-method (`POST`/`PUT`/`PATCH`/`DELETE`) requests on `/v2/*` are challenged the same mode-aware way. Under `AuthContext::Disabled` every denial — anonymous or not — collapses to the 404 (the `/v2/auth` realm is unsatisfiable there; no unsatisfiable challenge is ever emitted). This is a representation change only: the authority computation this entry describes (grants ∩ cap, no admin short-circuit) is unaffected.
- **Anonymous mint posture on `/v2/auth` (#48).** The `/v2/auth` MINT endpoint (distinct from the `/v2/*` consume-path challenge above) no longer 401s when **no** `Authorization: Basic` credential is presented — it mints an **anonymous** token via `OciTokenExchangeUseCase::exchange_anonymous`, so a challenge-driven anonymous `docker pull` of a **public** pull-through repo completes its token handshake instead of dead-ending at the mint. Authority bound: for each requested `repository:<name>:pull` scope, `pull` is granted **iff the repo is PUBLIC** — precisely when an anonymous (`actor = None`) `RepositoryAccessUseCase::resolve(Read)` succeeds; the mint-side predicate is **byte-identical to the consume-side read gate**, so a repo the anonymous caller could not read is never echoed as grantable. **Private repos, non-`pull` actions (`push`/`delete`), and `registry:catalog` are NEVER anonymously granted** — silently omitted from `access[]`. The minted JWT carries **`sub = Uuid::nil()`** (the anonymous sentinel): it owns no `GrantSubject::User` grants, so on consume its effective authority is `is_public`-read only (the token `access[]` is the ADR 0036 upper-bound cap intersected with the subject's grants — an empty grants leg confers **no ambient authority**). An all-private / all-`push` request still mints a valid **200** token with an empty `access[]` (the Distribution-Spec "anonymous ping" token), never a 401; the read gate for private repos stays at the resource endpoint (the `/v2/*` challenge posture above, unchanged). Malformed / empty `Basic` headers are treated as absent → anonymous. The Step-0 `service=`/`aud` gate, the `MAX_SCOPES` bound, and scope parsing are the **same** shared request-shape checks as the credentialed path (`validate_service_and_parse_scopes`), so the anonymous path cannot bypass them. **A future change that grants an anonymous token authority beyond public-repo `pull` — any private-repo grant, any `push`/`delete`/`admin`, or a non-nil `sub` — is a hard block.**
...
- **Allowed call paths / surfaces:** package-manager artifact surfaces — npm/pip/cargo and **maven/gradle** (`mvn deploy` / `gradle publish` deploy `PUT`s authenticate here via `Authorization: Basic base64(<anything>:<hort_pat_*>)`, username ignored, password = PAT validated as Entry 2; the Maven handler serves both `RepositoryFormat::Maven` and `RepositoryFormat::Gradle` repos on one wire protocol). Maven/Gradle introduce **no new auth mechanism** — they reuse this carrier unchanged. **OCI `/v2/*` consume surface:** the OCI bearer middleware (`crates/hort-http-oci/src/middleware/oci_auth.rs`) also accepts `Authorization: Basic <b64(user:token)>` as a carrier — the password is treated as the bearer token and validated the same two-stage way as an actual `Bearer` header (native `hort_pat_*`/`hort_svc_*` as Entry 2/4, or an IdP JWT in the password slot); the username is ignored, same as every other surface this entry covers. Active under **both** `AuthContext::Enabled` and `AuthContext::BearerOnly` (ADR 0045) — a `BearerOnly` deployment (no IdP, native tokens on) validates a directly-presented native token on `/v2/*` identically to `Enabled`. Basic stays a carrier, never an identity source, on this surface too.
- **Enforcement owner:** `Hort-enforced`.
...
- **Restrictions & caps:** `exp/nbf/iss/alg` validated, 30 s leeway; no `insecure_jwks_url` knob. Before the fetch, `internal::check_jwks_uri_bound` binds the discovery-supplied `jwks_uri` to the issuer origin on two legs: same HOST (case-insensitive; the load-bearing SSRF control — a compromised/MITM'd discovery document cannot redirect the fetch off-origin) and same SCHEME (equality, not an HTTPS-only floor — rejects an HTTPS issuer whose `jwks_uri` downgrades to plain HTTP on the same host, while the in-cluster http-issuer/http-jwks Keycloak deployment keeps working). Both legs are additive to the TLS-version pin + redirect cap on the shared client; a rejection on either leg collapses to the same JWKS-fetch error classification as any other fetch failure on that path — no new wire error or metric. JWKS refetch is single-flight (single-issuer: one refresh in flight process-wide; federation: one refresh in flight per issuer — a different issuer's refresh proceeds independently) and the cache lock is never held across the network fetch, so cached-kid validation stays uncontended while a refresh (up to ~20 s worst case) is in flight. A fresh-cache unknown-kid refetch is throttled by a cooldown — single-issuer: the `HORT_JWKS_EVICTION_BACKOFF_SECS` window (default 10 s, dual-role with the existing per-kid signature-mismatch eviction backoff); federation: a fixed 10 s constant (no config-surface knob). TTL/interval-stale refreshes are never gated by the cooldown; a genuinely rotated key is accepted within at most one cooldown window.
